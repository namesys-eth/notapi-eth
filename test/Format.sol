...
abstract contract Format {
    ...
    function Encode(bytes calldata _input) public pure returns (bytes32 _namehash, bytes memory _name) {
        ...
        _names[k] = _input[last:];
            for (uint256 j = 0; j <= k; ++j) {
                _name = bytes.concat(_name, bytes1(uint8(_names[j].length)), _names[j]);
                _namehash = keccak256(abi.encodePacked(_namehash, keccak256(_names[j])));
            }
        }
        _name = abi.encodePacked(_name, bytes1(0));
    }
